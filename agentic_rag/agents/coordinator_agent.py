...
# Import Azure AI Foundry comprehensive monitoring
from monitoring import azure_monitor
...
class CoordinatorAgent:
    ...
    def __init__(self):
        ...
        # Initialize Azure AI Foundry tracing system
        self._register_agents_with_tracing()
        
        # Cache for agent IDs - created once, reused many times
        self.agent_cache: Dict[str, str] = {}
        self.agents_initialized = False
    
    def _register_agents_with_tracing(self):
        """Register all agents with the Azure AI Foundry tracing system."""
    ...
    def execute_agent_with_streaming(self, agent_id: str, query: str, agent_name: str, 
        ...
        # Map agent_id to tracing agent name
        agent_tracing_name = {
            self.agent_cache.get('research', ''): 'research_agent',
            self.agent_cache.get('analysis', ''): 'analysis_agent',
            self.agent_cache.get('synthesis', ''): 'synthesis_agent'
        }.get(agent_id, 'unknown_agent')
        
        # Use comprehensive Azure AI Foundry monitoring with detailed agent tracking
        with azure_monitor.trace_agent(agent_tracing_name, "healthcare_agent", 
                                     agent_id=agent_id, 
                                     query=query,
                                     includes_tools=True) as agent_span:
            try:
                # Create message
                ...
                if run.status == "completed":
                    ...
                    if assistant_messages:
                        ...
                        azure_monitor.log_custom_event("agent_completed",
                ...
                # Log error event to Azure Monitor
                azure_monitor.log_custom_event("agent_error",
                                             agent_name=agent_name,
                                             agent_id=agent_id,
                                             error=str(e))
    ...
    def execute_parallel_workflow_with_streaming(self, query: str, 
        ...
        # Use comprehensive Azure AI Foundry monitoring with detailed workflow tracking
        with azure_monitor.trace_workflow("healthcare_multi_agent_workflow", 
                                        query=query,
                                        workflow_type="multi_agent_parallel",
                                        total_agents=3,
                                        includes_azure_search=True,
                                        application_name="healthcare-agentic-rag") as workflow_span:
            ...
            # Step 1: Research Agent (must run first) - includes Azure AI Search
            if progress_callback:
                progress_callback("🔍 Step 1: Research Agent - Healthcare Document Retrieval with Azure AI Search")
            
            # Add workflow event for research phase
            workflow_span.add_event("research_phase_started", {
                "phase": "research",
            ...
            # Add workflow event for research completion
            workflow_span.add_event("research_phase_completed", {
                "phase": "research",
                "agent": "research_agent",
            ...
            if progress_callback:
                ...
                progress_callback("📝 Step 3: Synthesis Agent - Response Generation with Code Interpreter")
                progress_callback("🔄 Running Analysis and Synthesis agents in parallel...")
            
            # Add workflow event for parallel phase
            workflow_span.add_event("parallel_phase_started", {
                "phase": "parallel_analysis_synthesis",
                "agents": ["analysis_agent", "synthesis_agent"],
                "includes_code_interpreter": True
            })
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                ...
                # Add workflow events for parallel phase completion
                workflow_span.add_event("parallel_phase_completed", {
                    "phase": "parallel_analysis_synthesis",
                    "analysis_status": analysis_result['status'],
                    "synthesis_status": synthesis_result['status'],
                    "analysis_content_length": len(analysis_result.get('content', '')),
                    "synthesis_content_length": len(synthesis_result.get('content', ''))
                })
            ...
            azure_monitor.log_metric("workflow_duration", duration, 
                                   workflow_name="healthcare_workflow",
                                   successful_agents=successful_agents,
                                   total_agents=3)
            
            # Log custom event
            azure_monitor.log_custom_event("workflow_completed",
                                         workflow_name="healthcare_workflow",
                                         duration=duration,
                                         successful_agents=successful_agents,
                                         total_agents=3)
